...
C**********************************************************************
      ...
      CHARACTER FNAME*12,FSTAT*3 ,fname1*20
      INCLUDE 'de4com.fi'
...
C:MyChange
      !open(unit=8, file='..\..\de4re\de4re\DEISA')
      open(unit=9, file='.\data\Save.txt')
      !open(unit=10, file='..\..\de4re\de4re\NSave')
      open(unit=30, file='.\data\Time.txt')
      open(unit=34, file='.\data\layer1.txt')
      open(unit=35, file='.\data\layer2.txt')
      open(unit=36, file='.\data\layer3.txt')
      open(unit=37, file='.\data\layer4.txt')
      open(unit=38, file='.\data\layer5.txt')
      open(unit=39, file='.\data\LayerAll.txt')
...
C  electrical conductivity at room temperature:
      ...
      RAB=ZSUB(K)+.5D0
      IZSUBK=RAB
      IF(IZSUBK.EQ.29) FITCAP(K)=3.D0  !第K种物质电阻率拟合系数
...
C  and not saved in 'DEISA3' (for any run):
      ISTART=0 !=0 开始一个new run；=1继续old run(保存在DEISA3中)
      TFIN=15.D0 !结束的时间   5.D0   changing
      NTFIN=100000000 !结束的时间步数   changing
      TPRIN=TIMPRI(1) !输出的时间
      NPRINT=0 !记录输出次数
C     DTPRIN=1.D18
...
CMy change
      TSAVE=0.01
C: define geometry, type of the left boundary, and target structure:
      IGEO=2          !changing
      IFLBND=0
      NZ=5
      NZEVEN=1
...
C:: set parameters of target layers:
      RZ0(1)=0.D0
...
C      IF(NTSAVE*(NTSLOI/NTSAVE).EQ.NTSLOI) ISAVE=1
CMy change
      IF(TSAVE*NSAVE.LE.TIME.AND.TIME.LE.TSAVE*(NSAVE+1)) ISAVE=1
      IF(ISAVE==1) IPRIN=1
...
C:MyChange      
      WRITE(9,9045)NSAVE,NTSLOI,TIME
      ISAVE=0
 1790 CONTINUE
C============================================================= end SAVE
...
C============================================================== end RUN
 ...
 2010 PRINT 9070
C:MyChange      
      WRITE(9,*)' Could not open file DEISA3 for ISTART.GE.1'
      !close(8)
      close(9)
      !close(10)
      close(30)
...
C:MyChange      
      WRITE(9,9020)IGEO,IFLBND,RZ0(1)
...
C:MyChange      
      WRITE(9,9030)NSUB(I),I,ASUB(NSUB(I)),ZSUB(NSUB(I))
...
C:MyChange      
      WRITE(9,9240)RAB,RAB1,J,K

      RAB=2.942D0*P00(I)
      RAB1=2.942D0*E00(I)/(11.206D0*ASUB(I))
      PRINT 9250,RAB,RAB1
C:MyChange      
      WRITE(9,9250)RAB,RAB1
...
C:MyChange      
      WRITE(9,9270)NRUN,LGEO(I1GEO),LLBND(J),RZ0(1),NTPRI,NJPRI,NTSAVE

      PRINT 9280,LNEU(IFN14+1),LNEU(IFN2+1)
     *,LAL(IFAL+1),LAL(IFP3+1),LAL(IFP14+1)
C:MyChange      
      WRITE(9,9280)LNEU(IFN14+1),LNEU(IFN2+1)
     *,LAL(IFAL+1),LAL(IFP3+1),LAL(IFP14+1)

      PRINT 9300,EIONB,WDRIV,TDRFIN
C:MyChange      
      WRITE(9,9300)EIONB,WDRIV,TDRFIN

      PRINT 9305,LHZ(IFHZ+1),HZ0,IFOPAC
C:MyChange      
      WRITE(9,9305)LHZ(IFHZ+1),HZ0,IFOPAC
...
C:MyChange      
      WRITE(9,9320)I,NSUB(I),J,AMZ(I),RAB,RAB1
     *,ROZ0(I),RZ0(I+1),XD0(I),XT0(I),XHE0(I),XH0(I),XB0(I)
...
C:MyChange      
      WRITE(9,9400)NTSLOI,TIME,DT,NDRIV,NTBAD
     *,NTVBAD,NTFLLE,NTFLLI,NTFLLR
...
C:MyChange
      WRITE(9,9420)PBLOLD,PBROLD,HBLOLD,HBROLD,TRLEX,TREX,EREX,ERIN
     *,TR(N),RORFU,RORFUM,TOTN2,TOTN14,EBLNC,RAB,TOTDHE3

C输出总的信息
      write(39,9454)EMT,PBLOLD,PBROLD,HBLOLD,HBROLD,TRLEX,TREX,EREX,ERIN
     *,TR(N),RORFU,RORFUM,TOTN2,TOTN14,EBLNC,RAB,TOTDHE3
 9454 format(1P47E13.4)
...
C:MyChange      
      WRITE(9,9430)I,ZSUB(NSUB(I)),AEOS,RAB
...
C:MyChange      
      WRITE(9,9410)(EMZ(I,K),K=1,14),EZDR(I),EZJL(I),EZFUS(I)
     *,EZCL(I),EZN14(I),EZN2(I),EZAL(I),EZP3(I)
     *,EZP14(I),EOUTW(I+1),EOUTE(I+1),EOUTI(I+1),EOUTR(I+1)
     *,EOUTHZ(I+1),EOUTAL(I+1),EOUTP3(I+1),EOUP14(I+1)
...
C:MyChange      
      WRITE(9,9435)TNUN2(I),TNUN14(I),TNUDHE3(I),EBLNC

C:将每一层的总参数输出      
      !write(fname1,'(I10)')I
      !open(33,file='.\data\layer'//trim(adjustl(fname1))//'.txt') !创建文件
      WRITE(33+I,9453)(EMZ(I,K),K=1,14),EZDR(I),EZJL(I),EZFUS(I)
     *,EZCL(I),EZN14(I),EZN2(I),EZAL(I),EZP3(I)
     *,EZP14(I),EOUTW(I+1),EOUTE(I+1),EOUTI(I+1),EOUTR(I+1)
     *,EOUTHZ(I+1),EOUTAL(I+1),EOUTP3(I+1),EOUP14(I+1)
     *,TNUN2(I),TNUN14(I),TNUDHE3(I),EBLNC
 9453 format(1P35E13.4)    
      
      JHI1=1
      IF(IFHZ.EQ.1) JHI1=3
      PRINT 9440,LAL3(JHI1)
C:MyChange      
      WRITE(9,9440)LAL3(JHI1)

      IF(I.NE.1) GOTO 820
      JRAB=0
      PRINT 9451,JRAB,R(1),U(1)
C:MyChange      
      WRITE(9,9451)JRAB,R(1),U(1)  !9451输出的J=0的值

 820  JI=NJZ(I)
      JF=NJZ(I+1)-1

      DO 950 J=JI,JF   !输出格点信息
      IF(NJPRI*(J/NJPRI).NE.J.AND.J.NE.JI.AND.J.NE.JF) GOTO 950  !NJPRI：设置输出间隔
      RAB=1.D0/V(J)
      
      write(fname1,'(I10)')NPRINT
      open ( 31, file = '.\data\step1='//trim(adjustl(fname1))//'.txt') !创建文件
      
      IF(IFHZ.EQ.1) THEN
        PRINT 9450,J,R(J+1),U(J+1),RAB,TE(J),TI(J),TR(J),HZ(J),P(J)
C:MyChange      
      WRITE(9,9450)J,R(J+1),U(J+1),RAB,TE(J),TI(J),TR(J),HZ(J),P(J)
      WRITE(31,9450)J,R(J+1),U(J+1),RAB,TE(J),TI(J),TR(J),HZ(J),P(J)
      
      ELSEIF(JHI1.EQ.1) THEN
        PRINT 9450,J,R(J+1),U(J+1),RAB,TE(J),TI(J),TR(J),HIER(J),P(J)
C:MyChange      
      WRITE(9,9450)J,R(J+1),U(J+1),RAB,TE(J),TI(J),TR(J),HIER(J),P(J)
      WRITE(31,9450)J,R(J+1),U(J+1),RAB,TE(J),TI(J),TR(J),HIER(J),P(J)

      ELSEIF(JHI1.EQ.2) THEN
        PRINT 9450,J,R(J+1),U(J+1),RAB,TE(J),TI(J),TR(J),HIEI(J),P(J)
C:MyChange      
      WRITE(9,9450)J,R(J+1),U(J+1),RAB,TE(J),TI(J),TR(J),HIEI(J),P(J)
      WRITE(31,9450)J,R(J+1),U(J+1),RAB,TE(J),TI(J),TR(J),HIEI(J),P(J)
...
C:MyChange      
      WRITE(9,9444)LAL1(NALPRI),LAL2(NALPRI)

      GOTO 962
 960  PRINT 9442,LAL1(NALPRI),LAL2(NALPRI)
C:MyChange      
      WRITE(9,9442)LAL1(NALPRI),LAL2(NALPRI)
 ...
 962  CONTINUE
      ...
      write(fname1,'(I10)')NPRINT
      open ( 32, file = '.\data\step2='//trim(adjustl(fname1))//'.txt') !创建文件
      
      IF(XB0(I).LT.XBFLOO) GOTO 976
      PRINT 9452,J,YI(J),CAPE(J),CAPI(J),CAPR(J)
     *,EALL(K),HIALL(K),XH(J),XB(J),RAB1
C:MyChange      
      WRITE(9,9452)J,YI(J),CAPE(J),CAPI(J),CAPR(J)
     *,EALL(K),HIALL(K),XH(J),XB(J),RAB1
      WRITE(32,9452)J,YI(J),CAPE(J),CAPI(J),CAPR(J)
     *,EALL(K),HIALL(K),XH(J),XB(J),RAB1
...
C9450 FORMAT(I4,1PE12.5,1P7E9.2)
 9450 FORMAT(I4,1PE12.5,1P7E10.2)
 9451 FORMAT(I4,1PE12.5,1PE10.2)
...
C**********************************************************************
      ...
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
c ---------------------------------------------------------------------
c     This routine calculates the values of the boundary pressure,             !边界压强、外部辐射温度、磁场
...
C: boundary pressure:   边界压强为0
      PBL=0.D0
      PBR=0.D0

C: external X-ray drive temperature:   边界X-ray驱动温度为0
      TRLEX=0.D0
      TREX=0.D0*TYME

C: boundary magnetic z-field:   边界磁场为HZ0
      HZBL=HZ0 
      HZBR=HZ0

C: total external pressure:        公式（4.9）
      PBLSUM=PBL+ASBOL*TRLEX**4/3.D0
      PBRSUM=PBR+ASBOL*TREX**4/3.D0
      IF(IFHZ.NE.1) RETURN !GOTO 290
...
C--------------------------------------------------------- begin NEWXDT
C: solve the thermonuclear burn equations:   解燃烧方程，求D、T、He、H、B在各层的比例
c IFBURN =-1 忽略燃烧，不检查点火条件
c       = 0 忽略燃烧，检查点火条件
c       = 1 当打开燃烧时媒介值？
c       = 2 计算燃烧，不检查停止条件
      ...
      DO 100 I=1,NZ
      JI=NJZ(I)          !I层的第一个cell的顺序数
      JF=NJZ(I+1)-1

      DO 100 J=JI,JF
      IF(IFDMFU(J).NE.1) GOTO 100     !IFDMFU(J)第J个cell中有无燃料，将IFMFU0(I)内的cell赋值为IFDMFU(J)
      XDJ=XD(J)
      QDTJ=QSDT(J)
      QDDJ=QSDD(J)
      QDHEJ=QSDHE(J)
...
C     QP3(J) -> QSDD(J),  QP14(J) -> QSDHE(J)   !!!
     ...
     +3.54D4*XDJ*XHE(J)*QDHEJ+8.38D4*XH(J)*XB(J)*QSBH(J))    !公式(6.21)
      QP3(J)=2.91D4*RAB*XDJ*XDJ*QDDJ/2.D0        !公式(6.22)
      QP14(J)=14.16D4*RAB*XDJ*XHE(J)*QDHEJ         !公式(6.23)
C: new abundances for DD group:
      RAB=DT/(V(J)*AMOL(I))
      RAB1=RAB*XDJ*QDDJ
      XT(J)=(XT(J)+.5D0*RAB1*XDJ)/(1.D0+RAB*XDJ*QDTJ)       !公式(3.32)
      XHE(J)=(XHE(J)+.5D0*RAB1*XDJ)/(1.D0+RAB*XDJ*QDHEJ)    !公式(3.33)
      XD(J)=XDJ*(1.D0-2.D0*RAB1-RAB*(XT(J)*QDTJ+               !公式(3.31)
     +XHE(J)*QDHEJ))
C: new abundances for BH group:        !公式(3.34)
...
C: boundary conditions, initial assignments:
      DT5=.5D0*DT
      CALL BNDVAL(TIME+DT5)   !处理边界压强、外加热温度、磁场
      DM(N1)=0.D0             !DM???
      IFVOCL=0
      R1OLD=R(1)
      U1OLD=U(1)
...
C: lay away for the work at boundaries:
 ...
 240  WN2(1)=PBL+ASBOL*TR(1)**4/3.D0
      ...
      IF(IFBURN.EQ.2) WN2(1)=WN2(1)+2.D0*(EAL(1)+EP3(1)+EP14(1))/3.D0 !公式（4.5）
 260  CONTINUE
...
C:MyChange      
      WRITE(9,9110)NTSLOI,TIME,R1OLD,U1OLD
...
C:MyChange      
      WRITE(9,9020)TIME,NTSLOI,J,HZ(J)
...
C**********************************************************************
      ...
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
c ---------------------------------------------------------------------
c     This routine calculates the full first elliptic integral. 第一类椭圆积分
...
C:MyChange      
      WRITE(9,511)D,TE,Y2,YT,YTN,F2,F1,HM
...
c =====================================================================
 ...
 200   WRITE(JCHAN,*) DUMP,DM,R,U,V,TE,TI,TR,HZ
     ...
     *,FLLE,FLLI,FLLR,TBURN0,XBFLOO
     *,CZDT,CZDTQ,CZDTVT,CZDDT,CZDRIV,CZVKIN,CZTKIN
       !WRITE(8,*) DUMP,DM,R,U,V,TE,TI,TR,HZ
...
C:MyChange      
      WRITE(9,511)NRCAL,XL,X,XR,H,FP,F2
